...
namespace Books.Foundation.Assets.Repo
{
    public class AssetRepository
    {
        ...
        {
            string cleanContent = this.CleanContent(content);
            if (cleanContent == null)
                return null;
            Asset asset = new Asset(assetType, location, cleanContent, contentType, site);
            return asset;
        }
        ...
        internal Asset CreateFromConfiguration(XmlNode node)
        {
            ...
            string site = XmlUtil.GetAttribute("site", node, null);
            string inlineValue = node.InnerXml;
            ...
            if (!Enum.TryParse<AssetType>(assetTypeString, true, out var assetType))
            {
            ...
            {
                if (!Enum.TryParse<ScriptLocation>(scriptLocationString, true, out var location))
                {
